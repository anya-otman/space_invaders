...
class PlayerTestCase(unittest.TestCase):
    def test_move_left(self):
        player = Player((100, 100), 600)
        player.move_left()

        player_x_coordinate = player.rect.x
        expected_player_x_coordinate = Player((95, 100), 600).rect.x
    ...
    def test_move_right(self):
        ...
        player_x_coordinate = player.rect.x
        expected_player_x_coordinate = Player((105, 100), 600).rect.x
        self.assertEqual(expected_player_x_coordinate, player_x_coordinate)

    def test_move_left_with_constraint(self):
        player = Player((-1, 0), 600)
        player.move_left()
